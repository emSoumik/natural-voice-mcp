...
When this skill is used through the Codex plugin, call `natural_voice_guidance` for the task and chosen register before drafting. It serves the same full skill and reference set bundled in this repository. The connected AI client writes the final response; the MCP server does not generate text or add facts. After drafting, call `lint_natural_voice` with the draft and register. Treat its flags as hints, not law. If the MCP server is unavailable, use these local references and `scripts/check.py` instead. Never send or post text without the user's authorization.
...
If a shell is available, run `python3 scripts/check.py --register <level> <file>` (or pipe text to stdin) as a final mechanical pass. It catches em dashes, banned words, stray capitals, bold overuse, and chatbot endings. Treat its output as hints, not law.
